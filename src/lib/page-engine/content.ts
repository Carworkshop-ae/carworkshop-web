...
export async function getRelatedSections(page: GeneratedPageRow, brandName: string | null): Promise<RelatedSections> {
  ...
  switch (page.template_type) {
    ...
    case 'brand_service': {
      // Same-brand cross-sell first: sibling brand_service pages for this
      // brand (excluding this page itself), then brand-scoped model
      // variants, only falling to brand-agnostic general services last.
      let services = (await findPublished({ template_type: 'brand_service', brand_id: page.brand_id, sameState: page.state })).filter(r => r.slug !== page.slug)
      if (services.length === 0) services = await findPublished({ template_type: 'brand_model_service', brand_id: page.brand_id, sameState: page.state })
      if (services.length === 0) services = await findPublished({ template_type: 'general_service', sameState: page.state })
    ...
    case 'brand_model_service': {
      // Sibling model-specific services first, then fall back to the
      // parent brand's brand_service pages, general_service as last resort.
      let services = (await findPublished({ template_type: 'brand_model_service', brand_id: page.brand_id, model_id: page.model_id, sameState: page.state })).filter(r => r.slug !== page.slug)
      if (services.length === 0) services = await findPublished({ template_type: 'brand_service', brand_id: page.brand_id, sameState: page.state })
      if (services.length === 0) services = await findPublished({ template_type: 'general_service', sameState: page.state })
